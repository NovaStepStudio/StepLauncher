...
- **Una tarea = UN solo MD (consolidación)**: si una petición o sesión de trabajo implica más de un cambio (varios archivos, varios módulos, varias funcionalidades) o más de un error relacionado (misma causa raíz, misma investigación), se documenta TODO JUNTO en un único `StepLauncher-Change-N.md` o `StepLauncher-Error-N.md` con subsecciones (p. ej. `### 1.`, `### 2.`, ...). NO crear un MD por cada cambio o archivo aislado: fragmenta el historial, dispara la numeración y hace ilegible la trazabilidad. Solo se abre una entrada nueva cuando es una tarea distinta o una causa raíz independiente.
