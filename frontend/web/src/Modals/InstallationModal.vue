...
function loaderLogLine(line: string): void {
    ...
    lastLogLine = line;
    loaderLogs.value = [...loaderLogs.value, line].slice(-40);
}
    ...
    try {
        if (phase.value === 'setup') {
            ...
            const active = dl.find(
                (d: DownloadInfo) =>
                    d.version === selectedVersion.value &&
                    ['pending', 'downloading', 'paused', 'verifying', 'redownloading'].includes(d.state)
            );
            if (active) {
                    ...
                    <div class="InstallationModal_Body">
                                    ...
                                    <div v-else class="InstallationModal_List">
                                        <template v-for="group in versionGroups" :key="group.type">
                                            ...
                                            >
                                                ...
                                                <em v-if="isLatest(v.id, v.type)" class="InstallationModal_VersionBadge latest">ÚLTIMA</em>
                        ...
                        <template v-else-if="phase === 'installing'">
                            <div class="InstallationModal_Progress">
                                    ...
                                    <div v-if="(progress?.sections ?? []).length" class="InstallationModal_DetailBlock">
                                        ...
                                        <div class="InstallationModal_DetailList">
                                            ...
                                            >
                                                ...
                                                <span class="InstallationModal_DetailRowMeta">{{ s.doneFiles }}/{{ s.totalFiles }} archivos · {{ fmtMb(s.mbDownloaded) }}/{{ fmtMb(s.mbTotal) }} MB</span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div v-if="loaderPhase !== 'idle'" class="InstallationModal_LoaderStatus" :class="{ done: loaderPhase === 'done', error: loaderPhase === 'error' }">
                                    <span v-if="loaderPhase === 'running'" class="InstallationModal_Spinner" aria-hidden="true"></span>
                                    <IconCheck v-else-if="loaderPhase === 'done'" />
                                ...
                                <div v-if="loaderPhase === 'running'" class="InstallationModal_LoaderBody">
                                    ...
                                    <div v-if="loaderLogs.length" class="InstallationModal_LoaderLog">
...
@use '../Styles/InstallationModal.scss';
</style>
