...
export const launchPrepare = ref<{ active: boolean; current: number; total: number; label: string }>({
    active: false,
    current: 0,
    total: 0,
    label: '',
...
export const launchPrepareText = computed(() => {
    ...
    if (!p.active) return '';
    if (p.total > 0 && p.label) return `Descargando ${p.label} (${p.current}/${p.total})…`;
    if (p.total > 0) return `Descargando archivos faltantes (${p.current}/${p.total})…`;
    return 'Descargando archivos faltantes…';
});

function resetLaunchPrepare(): void {
    launchPrepare.value = { active: false, current: 0, total: 0, label: '' };
}
...
function subscribeWindowHideRestore() {
...
async function maybeShowWindow() {
    try {
...
async function hideOnLaunchIfEnabled() {
    try {
