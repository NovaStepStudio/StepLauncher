...
const instDl = computed(() => downloads.value[props.name]);

const isInstalled = (v: string): boolean => detailOf(props.name)?.meta?.versions?.includes(v) ?? false;
...
const SECTION_LABELS: Record<string, string> = {
    ...
    // la emite como "assets"). Se mantiene alias por compatibilidad.
    asset_index: 'Assets',
    java: 'Java',
...
function pickDefaultVersion() {
    const free = (v: VersionEntry) => !isInstalled(v.id);
    const release = versions.value.find((v) => v.type === 'release' && free(v));
    const snap = versions.value.find((v) => v.type === 'snapshot' && free(v));
    const fallback = versions.value.find(free);
...
async function onInstall() {
    ...
    errorMessage.value = '';
    const err = await addVersion(props.name, selectedVersion.value);
    if (err) {
        phase.value = 'error';
    ...
    dlId.value = downloads.value[props.name]?.dlId ?? '';
    if (!dlId.value) {
        phase.value = 'error';
...
async function syncFromBackend() {
    try {
        ...
        // si la instancia tiene una descarga en curso (pending/downloading/
        // paused/verifying/redownloading) se retoma su visualización; si el
        // estado anterior era un resumen cerrado (done/error) de una
        // instalación ya terminada, se restablece al menú de instalación.
        const ACTIVE = ['pending', 'downloading', 'paused', 'verifying', 'redownloading'];
        const st = instDl.value;
        if (st && ACTIVE.includes(st.state)) {
            const adopting = st.dlId !== dlId.value || phase.value !== 'installing';
            if (adopting) {
                pendingLoaderInstall.value = false;
                if (st.version) selectedVersion.value = st.version;
                phase.value = 'installing';
            }
            dlId.value = st.dlId;
            const pr = await GetDownloadStatus(st.dlId);
            if (pr) applyProgress(pr as DownloadProgress);
            return;
        }
        // Sin descarga de versión activa pero con instalación de modloader en
        // curso (abierta desde el detalle con «Panel»): se adopta su estado y
        // los eventos modloader_* que sigan llegando actualizan la vista.
        const ld = loaderDlOf(props.name);
        if (ld && ['resolving', 'downloading', 'installing'].includes(ld.phase)) {
            pendingLoaderInstall.value = true;
            loaderPhase.value = 'running';
            loaderStatus.value = ld.message || 'Instalando modloader…';
            loaderProgress.value = ld.progress ?? 0;
            loaderTotal.value = ld.total ?? 0;
            const loaderInfo = LOADERS.find((l) => l.id === ld.loader);
            if (loaderInfo) {
                selectedLoader.value = loaderInfo.id;
                loaderVersion.value = ld.loaderVersion;
            }
            phase.value = 'installing';
            return;
        }
...
<template>
    <Teleport to="body">
        <Transition name="InstDl">
            <div v-if="visible" class="InstDl_Overlay" @click.self="close">
                <div class="InstDl_Dialog">
                    <div class="InstDl_Head">
                        <span class="InstDl_Icon">
                            <img v-if="phase !== 'setup'" :src="selectedLoaderIcon" alt="" />
                            <IconDownload v-else stroke="2" />
                        </span>
                        <div class="InstDl_Titles">
                            <h3>Instalar en la instancia</h3>
                            <p>{{ phase === 'setup' ? props.name : selectedVersion ? `Descargando ${selectedVersion}${selectedLoader !== 'vanilla' ? ` · ${selectedLoader}` : ''}` : 'Descargando…' }}</p>
                        </div>
                        <button class="InstDl_Close" title="Cerrar" @click="close">
                            <IconX stroke="2" />
                        </button>
                    </div>

                    <div class="InstDl_Body">
                        <template v-if="phase === 'setup'">
                            <div class="InstDl_Setup">
                                <section class="InstDl_Panel main">
                                    <div class="InstDl_PanelHead">
                                        <span class="InstDl_Label">Versión</span>
                                        <span v-if="manifestLoaded" class="InstDl_Count">{{ visibleVersionCount }} versiones</span>
                                    </div>

                                    <div class="InstDl_Search">
                                        <IconSearch />
                                        <input
                                            v-model="query"
                                            type="text"
                                    ...
                                    <div class="InstDl_FilterTabs">
                                        <button
                                            v-for="f in FILTER_TABS"
                                            :key="f.id"
                                            class="InstDl_FilterTab"
                                            :class="{ active: versionFilter === f.id }"
                                            @click="versionFilter = f.id"
                                        >{{ f.label }}</button>
                                    </div>

                                    <div v-if="loadingManifest || !manifestLoaded" class="InstDl_Loading">
                                        <span class="InstDl_Spinner" aria-hidden="true"></span>
                                        <span>Cargando versiones desde el manifest de Mojang…</span>
                                    </div>

                                    <div v-else-if="manifestError" class="InstDl_Notice error">
                                        <IconAlertTriangle />
                                        <span>No se pudo cargar el manifest: {{ manifestError }}</span>
                                    </div>

                                    <div v-else class="InstDl_List">
                                        <template v-for="group in versionGroups" :key="group.type">
                                            <div class="InstDl_GroupHead">
                                            ...
                                            >
                                                <span class="InstDl_VersionId">{{ v.id }}</span>
                                                <em v-if="isInstalled(v.id)" class="InstDl_VersionBadge installed">REINSTALAR</em>
                                                <em v-else-if="isLatest(v.id, v.type)" class="InstDl_VersionBadge latest">ÚLTIMA</em>
                                                <em v-else-if="v.type === 'snapshot'" class="InstDl_VersionBadge snap">SNAPSHOT</em>
                                                <em v-else-if="v.type !== 'release'" class="InstDl_VersionBadge legacy">ANTIGUA</em>
                                                <IconCheck v-if="v.id === selectedVersion" class="InstDl_VersionCheck" stroke="2.4" />
                                            </button>
                                        </template>
                                        <div v-if="!versionGroups.length" class="InstDl_Empty">
                                            <IconSearch stroke="2" />
                                            <span>Sin resultados para «{{ query }}»</span>
                                        </div>
                                    </div>

                                    <div v-if="isInstalled(selectedVersion)" class="InstDl_Notice info">
                                        <IconAlertTriangle />
                                        <span>Esta versión ya está instalada en la instancia: <strong>se volverá a descargar</strong> (reinstalación).</span>
                                    </div>
                                </section>

                                <section class="InstDl_Panel side">
                                    <div class="InstDl_PanelHead">
                                        <span class="InstDl_Label">Modloader</span>
                                        <span v-if="selectedVersion" class="InstDl_Count">para {{ selectedVersion }}</span>
                                    </div>

                                    <div class="InstDl_Loaders">
                                        <button
                                            v-for="l in LOADERS"
                                            :key="l.id"
                                            class="InstDl_Loader"
                                        ...
                                        >
                                            ...
                                            <span class="InstDl_LoaderName">
                                                {{ l.name }}
                                                <em v-if="compatOf(l.id)?.status === 'empty'" class="InstDl_LoaderLack">No disponible para {{ selectedVersion }}</em>
                                                <em v-else-if="compatOf(l.id)?.status === 'error'" class="InstDl_LoaderLack">No se pudo verificar {{ selectedVersion }}</em>
                                                <em v-else>{{ l.tag }}</em>
                                            </span>
                                            <span v-if="compatOf(l.id)?.status === 'loading'" class="InstDl_LoaderSpinner">
                                                <span class="InstDl_Spinner" aria-hidden="true"></span>
                                            </span>
                                            <IconCheck v-else-if="isLoaderSupported(l.id) && selectedLoader === l.id" class="InstDl_LoaderCheck" stroke="2.4" />
                                        </button>
                                    </div>

                                    <div v-if="selectedVersion && (selectedLoaderStatus === 'empty' || selectedLoaderStatus === 'error')" class="InstDl_Unavailable">
                                        <IconAlertTriangle />
                                        <span>{{ selectedLoaderHint }}</span>
                                    </div>

                                    <label class="InstDl_Toggle">
                                        <span class="InstDl_ToggleBox">
                                            <input v-model="manualLoaderVersion" type="checkbox" />
                                            <span class="InstDl_ToggleTrack"><span></span></span>
                                        </span>
                                        <span class="InstDl_ToggleText">
                                    ...
                                    <template v-if="selectedLoader !== 'vanilla' && selectedVersion">
                                        <template v-if="manualLoaderVersion">
                                            <div v-if="selectedLoaderStatus === 'loading'" class="InstDl_LoaderVersion loading">
                                                <span class="InstDl_Spinner" aria-hidden="true"></span>
                                                <span>Buscando versiones de {{ selectedLoader }}…</span>
                                            </div>
                                            <div v-else-if="selectedLoaderStatus === 'empty'" class="InstDl_LoaderVersion empty">
                                                <IconAlertTriangle />
                                                <span>No hay versiones de {{ selectedLoader }} para {{ selectedVersion }}.</span>
                                            </div>
                                            <div v-else-if="selectedLoaderStatus === 'error'" class="InstDl_LoaderVersion empty">
                                                <IconAlertTriangle />
                                                <span>No se pudo consultar {{ selectedLoader }} para {{ selectedVersion }}.</span>
                                            </div>
                                            <div v-else class="InstDl_LoaderVersion">
                                                <span class="InstDl_Label inner">Todas las versiones</span>
                                                <select
                                                    v-model="loaderVersion"
                                                    class="SsSel InstDl_Select"
                                                >
                                        ...
                                        <template v-else>
                                            ...
                                            <div v-else-if="selectedLoaderStatus === 'ok'" class="InstDl_AutoVersion">
                        ...
                        <template v-else-if="phase === 'installing'">
                            <div class="InstDl_Progress">
                                <div class="InstDl_StateRow">
                                    <span class="InstDl_StateVersion">{{ selectedVersion }}<template v-if="selectedLoader !== 'vanilla'"> · {{ selectedLoader }} {{ effectiveLoaderVersion }}</template></span>
                                </div>

                                <template v-if="loaderPhase !== 'running'">
                                <div class="InstDl_Ring" :class="ringClass">
                                    <svg viewBox="0 0 120 120" class="InstDl_RingSvg" aria-hidden="true">
                                        <circle class="InstDl_RingTrack" cx="60" cy="60" r="52" />
                                        <circle
                                            class="InstDl_RingFill"
                                            cx="60"
                                            cy="60"
                                            r="52"
                                    ...
                                    <span v-if="ringMode === 'percent'" class="InstDl_RingPct">
                                        {{ fmt(clampPct(progress?.percent ?? 0), 0) }}<em>%</em>
                                    </span>
                                    <span v-else-if="ringMode === 'busy'" class="InstDl_RingCenter">
                                        <span class="InstDl_Spinner" aria-hidden="true"></span>
                                    </span>
                                    <span v-else class="InstDl_RingCenter done">
                                        <IconCheck stroke="2.6" />
                                    </span>
                                </div>

                                <div class="InstDl_Step">
                                    <span class="InstDl_StepBadge" :class="stateBadge(downloadState).tone">
                                        <span class="dot"></span>
                                        {{ stateBadge(downloadState).text }}
                                    </span>
                                    <span v-if="stepHint" class="InstDl_StepHint">{{ stepHint }}</span>
                                </div>

                                <div v-if="downloadState === 'verifying' || downloadState === 'redownloading'" class="InstDl_VerifyCount">
                                ...
                                <div v-if="ringMode === 'percent'" class="InstDl_FriendlyLine">
                                ...
                                <button class="InstDl_DetailsToggle" :class="{ open: detailsOpen }" @click="detailsOpen = !detailsOpen">
                                    <IconChevronDown stroke="2" />
                                    {{ detailsOpen ? 'Ocultar detalles' : 'Detalles técnicos' }}
                                </button>

                                <div v-if="detailsOpen" class="InstDl_Details">
                                    <div class="InstDl_DetailStats">
                                        <span class="InstDl_DetailStat">
                                    ...
                                    <div v-if="progress?.currentFile" class="InstDl_DetailBlock">
                                        <div class="InstDl_DetailTitle">Descargando</div>
                                        <div class="InstDl_DetailCurrent">
                                            <span class="InstDl_DetailCurrentName">{{ progress!.currentFile }}</span>
                                            <div class="InstDl_DetailBar">
                                                <div class="InstDl_DetailBarFill" :style="{ width: clampPct(progress!.currentProgress) + '%' }"></div>
                                            </div>
                                            <span class="InstDl_DetailCurrentMeta">
                                    ...
                                    <div v-if="(progress?.sections ?? []).length" class="InstDl_DetailBlock">
                                        ...
                                        <div class="InstDl_DetailList">
                                            <div
                                                ...
                                                :key="s.name"
                                                class="InstDl_DetailRow"
                                                :class="{ done: s.totalFiles > 0 && s.doneFiles >= s.totalFiles }"
                                            >
                                                <div class="InstDl_DetailRowHead">
                                                    <span class="InstDl_DetailRowLabel">{{ sectionLabel(s.name) }}</span>
                                                    <em>{{ s.totalFiles ? fmt(s.doneFiles / s.totalFiles * 100, 0) : 0 }}%</em>
                                                </div>
                                                <div class="InstDl_DetailBar">
                                                    <div
                                                        class="InstDl_DetailBarFill"
                                                        :style="{ width: s.totalFiles ? clampPct(s.doneFiles / s.totalFiles * 100) + '%' : '0%' }"
                                                    ></div>
                                                </div>
                                                <span class="InstDl_DetailRowMeta">{{ s.doneFiles }}/{{ s.totalFiles }} archivos · {{ fmtMb(s.mbDownloaded) }}/{{ fmtMb(s.mbTotal) }}</span>
                                                <span v-if="s.name === 'java'" class="InstDl_DetailRowHint">
                                    ...
                                    <div v-if="(progress?.activeFiles ?? []).length" class="InstDl_DetailBlock">
                                        ...
                                        <div class="InstDl_DetailList">
                                            <div v-for="f in (progress!.activeFiles ?? []).slice(0, 8)" :key="f.name" class="InstDl_DetailFile">
                                                <div class="InstDl_DetailFileHead">
                                                    <span class="name">{{ f.name }}</span>
                                                    <em>{{ fmt(clampPct(f.percent), 0) }}%</em>
                                                </div>
                                                <div class="InstDl_DetailFileBar">
                                                    <div class="InstDl_DetailFileBarFill" :style="{ width: clampPct(f.percent) + '%' }"></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>

                                    <div v-if="(progress?.queuedCount ?? 0) > 0" class="InstDl_DetailBlock">
                                        <div class="InstDl_DetailTitle">En cola · {{ progress!.queuedCount }}</div>
                                        <div class="InstDl_DetailChips">
                                            <span v-for="name in (progress?.queuedPreview ?? []).slice(0, 5)" :key="name" class="InstDl_DetailChip">{{ name }}</span>
                                            <span v-if="(progress?.queuedPreview ?? []).length < progress!.queuedCount" class="InstDl_DetailChip more">
                                                +{{ progress!.queuedCount - (progress?.queuedPreview ?? []).length }}
                                            </span>
                                        </div>
                                    </div>

                                    <div v-if="(progress?.log ?? []).length" class="InstDl_DetailBlock">
                                        <div class="InstDl_DetailTitle">Registro</div>
                                        <div class="InstDl_DetailLog">
                                ...
                                <template v-else>
                                    ...
                                    <div class="InstDl_LoaderStatus running">
                                        <span class="InstDl_Spinner" aria-hidden="true"></span>
                                        <div class="InstDl_LoaderStatusText">
                                            <span class="title">Instalando {{ selectedLoader }} {{ effectiveLoaderVersion }}</span>
                                            <span class="detail">{{ loaderStatus }}</span>
                                        </div>
                                    </div>

                                    <div class="InstDl_LoaderBody">
                                        <div v-if="loaderTotal > 0" class="InstDl_LoaderBar">
                                            <div
                                                class="InstDl_LoaderBarFill"
                                        ...
                                        <div v-if="loaderTotal > 0" class="InstDl_LoaderParts">
                                            <span>Descargando parte {{ fmt(loaderProgress, 0) }} de {{ fmt(loaderTotal, 0) }}</span>
                                        </div>
                                        <div ref="loaderLogEl" class="InstDl_LoaderLog">
                                            <span v-for="(line, li) in loaderLogs" :key="li">{{ line }}</span>
                                        </div>
                                    </div>
                                </template>

                                <div v-if="errorMessage" class="InstDl_Notice error">
                                    <IconAlertTriangle />
                        ...
                        <template v-else-if="phase === 'done'">
                            <div class="InstDl_Done">
                                <span class="InstDl_DoneIcon"><IconCheck stroke="2.6" /></span>
                                <h4>
                                    {{ loaderPhase === 'done' ? '¡Listo!' : 'Instalación completada' }}
                                </h4>
                                <p>
                                    <template v-if="loaderPhase === 'done'">
                                        {{ selectedVersion }} con {{ selectedLoader }} {{ effectiveLoaderVersion }} está instalado en la instancia y listo para jugar.
                                    </template>
                                    <template v-else>
                                        La versión {{ selectedVersion }} se descargó correctamente en la instancia.
                                    </template>
                                </p>
                                <div v-if="finalSummary" class="InstDl_Summary">
                                    <div>
                        ...
                        <template v-else>
                            <div class="InstDl_Error">
                                <span class="InstDl_ErrorIcon"><IconAlertTriangle stroke="2.4" /></span>
